...
const timeSinceRelease = ( date: string, time: string ) => {
	return 'Time Since Release: ' + TimeDiff( new Date( `${ date } ${ time }` ), new Date( 'November 7, 2023 20:00' ) );
}

const timePreparingRelease = ( date: string, time: string ) => {
	return 'Time Preparing Release: ' + TimeDiff( new Date( 'November 8, 2023 18:05' ), new Date( `${ date } ${ time }` ),  );
}
	...
	<Slide transition={transition}>
		<FlexBox height="100%" flexDirection="column" justifyContent="space-between">
			<FlexBox flexDirection="row" font>
				<Heading fontSize="text" color="primary">{ date }</Heading>
				<Heading fontSize="text" color="primary">{ time }</Heading>
			</FlexBox>
			...
			<FlexBox flexDirection="column">
				<Heading fontSize="text" color="primary">
					{ since ? timeSinceRelease( date, time ) : timePreparingRelease( date, time ) }
				</Heading>
			</FlexBox>
		</FlexBox>
		<Notes>{ notes }</Notes>
